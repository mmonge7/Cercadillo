// Generado por scripts/build-content-data.mjs a partir de src/content. No editar a mano.
export const chapters = [
  {
    "id": "01-marco-geografico",
    "order": 1,
    "number": 1,
    "title": "Marco Geográfico, Territorial y Contexto Histórico General",
    "dek": "Localización, orografía, hidrología y despoblados del término municipal de Moriscos.",
    "readingMinutes": 9,
    "headings": [
      {
        "slug": "localizacion-en-el-alfoz-salmantino",
        "text": "Localización en el alfoz salmantino"
      },
      {
        "slug": "orografia-de-la-penillanura",
        "text": "Orografía de la penillanura"
      },
      {
        "slug": "hidrologia-arroyos-pontones-y-la-charca-municipal",
        "text": "Hidrología: arroyos, pontones y la charca municipal"
      },
      {
        "slug": "terminos-redondos-y-despoblados-historicos",
        "text": "Términos redondos y despoblados históricos"
      }
    ],
    "content": "## Localización en el alfoz salmantino\n\nMoriscos se halla enclavado en el cuadrante nororiental de la provincia de Salamanca, integrado en la comarca histórica y agrocultural de **La Armuña** (concretamente en la subcomarca de La Armuña Alta). La villa dista entre 8 y 10 kilómetros de la capital provincial —fijándose la distancia de referencia en unos **9 km al este-noreste**—, lo que la sitúa en la corona metropolitana inmediata del alfoz salmantino. Administrativamente es un municipio independiente adscrito al partido judicial de Salamanca y agrupado en la **Mancomunidad La Armuña** para la gestión de servicios comunes.\n\nEl término municipal abarca una superficie de **12,08 km²**. Limita al norte con Gomecello, San Cristóbal de la Cuesta y La Vellés; al sur con Cabrerizos; al este con Aldealengua; y al oeste con Villares de la Reina y Castellanos de Moriscos. Esta posición convierte a Moriscos en un espacio de transición entre la meseta cerealista armuñesa y las terrazas fluviales que descienden hacia la vega del Tormes.\n\n## Orografía de la penillanura\n\nEl término responde a las características de la **penillanura salmantina**: una fisonomía llana y suavemente ondulada, con horizontes abiertos, moldeada por erosión sobre materiales sedimentarios. El municipio se sitúa a una altitud media de entre **844 y 848 metros**, destacando como punto más alto el **Vértice Geodésico \"Andorra\"**, a **871,464 metros**. Desde esta cima, el terreno cae hacia la vega del Tormes (cota aproximada de 778 m), generando un desnivel de unos **93 metros** entre la meseta y los escarpes fluviales.\n\nLa microtopografía local conserva promontorios de valor estratégico histórico, como el **Teso de La Cabaña** y el alto de **El Parapeto**, que dominan los accesos a la capital. El callejero rural preserva topónimos de gran valor etnográfico: **Carrelavieja** (traza de un antiguo camino), **La Pardaleja** (por los tonos pardos del suelo o la abundancia de aves pardales) y **La Andorra** (topónimo de raíz arábiga que designa terrenos boscosos o escarpados).\n\n## Hidrología: arroyos, pontones y la charca municipal\n\nLa red hidrológica es modesta y estacional, articulada por pequeños cauces de régimen pluvial. Destaca el **Arroyo de Los Moriscos**, que atraviesa el casco urbano vecino de Aldealengua con un cauce estrecho y encajonado. Se enriquece con la confluencia de tres arroyos locales: **Vallejón** (valle escorrencial), **Gandil** (topónimo árabe conservado solo en esta zona, derivado de *candil* o lámpara) y **Serranilla** (prolongación de la elevación de La Serrada).\n\nSobre la antigua **Calzada de Medina**, el tránsito de caballerías exigía salvar el regato local mediante un característico **pontón de piedras**. El elemento hidrológico más emblemático de la vida comunitaria fue la **charca municipal o abrevadero**, situada históricamente en el centro del pueblo (frente al actual parque público), donde abrevaban las yuntas de bueyes. La confluencia de arroyos era, además, destino habitual de meriendas populares durante el **Lunes de Agua**. En el ámbito doméstico existía un marcado contraste entre la elevada dureza de las aguas subterráneas locales (superiores a los 40º) y el agua blanda que los vecinos buscaban en las fuentes de la vega de **La Flecha** para cocer bien las legumbres.\n\n## Términos redondos y despoblados históricos\n\nEl Catastro del Marqués de la Ensenada (1749-1752) registra un modelo de organización del suelo caracterizado por la atomización y la presencia de **términos redondos** (fincas cerradas de propiedad privada o eclesiástica) y alquerías asociadas. La presión fiscal, los conflictos bélicos y la expansión de los latifundios nobiliarios y eclesiásticos —destacando el Cabildo de la Catedral de Salamanca y los agustinos calzados— condujeron al despoblamiento de numerosas aldeas medievales, absorbidas por municipios matrices como Moriscos.\n\nEl término integra hoy varios despoblados: **Granja Capea**, antigua alquería hoy con cero habitantes censados; **La Estación**, surgida con el ferrocarril de 1877 y hoy fosilizada como apeadero; y los antiguos despoblados de **El Hoyo** y **La Cruz**. El Hoyo posee un significado identitario especial: su perfil orográfico quedó integrado en la **heráldica oficial del escudo municipal**, junto a la representación botánica de los zumacales."
  },
  {
    "id": "02-toponimia-repoblacion",
    "order": 2,
    "number": 2,
    "title": "Toponimia, Repoblación Medieval y Diversidad Etnocultural (Siglos XI-XIV)",
    "dek": "El origen etimológico de 'Moriscos', la repoblación de 1100 y la fundación de Castellanos de Moriscos.",
    "readingMinutes": 10,
    "headings": [
      {
        "slug": "la-paradoja-cronologica-del-toponimo",
        "text": "La paradoja cronológica del topónimo"
      },
      {
        "slug": "de-maurus-a-morisco-evolucion-linguistica",
        "text": "De *Maurus* a *Morisco*: evolución lingüística"
      },
      {
        "slug": "la-repoblacion-de-1100-alfonso-vi-y-ramon-de-borgona",
        "text": "La repoblación de 1100: Alfonso VI y Ramón de Borgoña"
      },
      {
        "slug": "el-conde-vela-de-aragon-y-castellanos-de-moriscos",
        "text": "El conde Vela de Aragón y Castellanos de Moriscos"
      },
      {
        "slug": "fiscalidad-y-administracion-siglos-xii-xiv",
        "text": "Fiscalidad y administración (siglos XII-XIV)"
      }
    ],
    "content": "## La paradoja cronológica del topónimo\n\nEn la cultura popular, \"morisco\" remite de inmediato a las pragmáticas de conversión forzosa dictadas por los Reyes Católicos en 1502 y por Carlos I entre 1525 y 1526, proceso que culminó en el decreto de expulsión general firmado el **9 de abril de 1609** por Felipe III y el Duque de Lerma. Sin embargo, la documentación medieval salmantina revela una paradoja: la denominación del asentamiento salmantino es más de tres siglos anterior a la formalización del vocablo *morisco*. Las primeras referencias paleográficas datan del **siglo XIV**, cuando la aldea aparece registrada en singular como «Morisco». Esta constatación demuestra que el topónimo se hallaba consolidado en la toponimia de La Armuña mucho antes de que la burocracia acuñase la palabra hacia 1517 para designar a los mudéjares bautizados.\n\n## De *Maurus* a *Morisco*: evolución lingüística\n\nEl vocablo deriva de la raíz latina *Maurus*, que calificaba a los habitantes de la Mauritania romana. La evolución fónica dio *moro*, complementado con el sufijo diminutivo *-isco*, dando *Mauriscos* y después *Morisco*. Como documentan Sebastián de Covarrubias (*Tesoro de la lengua castellana*, 1611) y Baltasar Salazar de Mendoza, el sufijo tenía valor diminutivo: «pequeño moro» o «medio-moro». En la Edad Media, «Morisco» aplicado a un lugar designaba la heredad de un individuo de filiación hispanomusulmana o de un pequeño grupo de origen islámico con estatuto diferenciado de los cristianos viejos. La forma plural «Moriscos» se consolidó a medida que la administración fue agregando alquerías y dehesas colindantes.\n\nLa génesis del poblamiento se vincula a contingentes **mudéjares, mozárabes y grupos norteafricanos** que permanecieron en la penillanura tras el repliegue de las fronteras andalusíes, calificados historiográficamente como «proto-moriscos». Topónimos afines como Morille o Moriñigo testimonian este mismo fenómeno en la provincia. Estos pobladores mantuvieron sus técnicas agrícolas y el manejo de la molienda hidráulica, acogidos al Fuero de Salamanca a cambio de tributos.\n\n## La repoblación de 1100: Alfonso VI y Ramón de Borgoña\n\nTras la conquista de la taifa de Toledo (1086) por Alfonso VI, la frontera leonesa se desplazó hacia el Tajo, dejando la franja entre el Duero y el Sistema Central en posición estratégica. Hacia el año **1100**, el conde **Ramón de Borgoña**, yerno de Alfonso VI y esposo de la infanta doña Urraca, dirigió una vasta campaña de repoblación bajo el modelo de las **Comunidades de Villa y Tierra**, otorgando al Concejo de Salamanca jurisdicción sobre un amplio alfoz rural.\n\nEl proceso atrajo a colonos francos, asturleoneses, gallegos, vasconavarros, riojanos y castellanos. La figura eclesiástica más relevante fue el obispo **Jerónimo de Perigord**, antiguo compañero de armas del Cid, nombrado obispo de la restaurada diócesis salmantina en 1102. Impulsó la Catedral Vieja y vinculó su figura a la devoción del **Cristo de las Batallas**, imagen románica venerada hoy en Castellanos de Moriscos.\n\n## El conde Vela de Aragón y Castellanos de Moriscos\n\nLos repobladores castellanos fueron organizados bajo el mando de **Vela de Aragón**, conde de Álava, quien erigió una «puebla de castellanos» junto a los terrenos ya ocupados por la comunidad de origen mudéjar. Esta fundación dio origen al vecino municipio de **Castellanos de Moriscos**, documentado en el siglo XIII como «Castellanos de Morisco» —un testimonio lingüístico que confirma la preexistencia del núcleo de Moriscos—. Ambos cascos urbanos, distantes apenas **1,39 kilómetros**, mantuvieron durante siglos una segregación espacial de cultivos: los cristianos viejos castellanos con propiedad hidalga o pechado ordinario, y los habitantes de Moriscos vinculados a huertas, regadío estacional y molienda en la vega del Tormes.\n\n## Fiscalidad y administración (siglos XII-XIV)\n\nBajo Fernando II de León (1157-1188), Moriscos se consolidó como núcleo estable, reflejado en las primeras estructuras de cantería de su iglesia parroquial de **San Pedro Apóstol**, con elementos de tradición mudéjar. El Concejo de Salamanca organizó su alfoz en **Cuartos**: el de la Armuña, el de **Villoria** (o Valdevilloria), el de Peña del Rey y el de Baños. Moriscos quedó adscrito al **Cuarto de Villoria** —de ahí su denominación histórica «Moriscos de Valdevilloria»—, mientras Castellanos de Moriscos quedó en el Cuarto de la Armuña.\n\nLa representación vecinal ante el concejo se articulaba mediante el **sexmero**, encargado de supervisar una compleja fiscalidad: **alcabalas** (impuesto sobre el tráfico mercantil), **pontazgos** (derecho de paso sobre puentes y pontones), **sisas** (gravamen sobre víveres básicos) y **pechos** (tributos directos del estado llano). El documento clave para reconstruir esta etapa es el **«Libro de todos los préstamos»** de la Catedral de Salamanca (1265), bajo Alfonso X el Sabio, donde la parroquia de Moriscos figura registrada con sus cuantías en grano, maravedís y gallinas —confirmación documental de la consolidación comunitaria en pleno siglo XIII."
  },
  {
    "id": "03-geografia-rural",
    "order": 3,
    "number": 3,
    "title": "Geografía Rural, Caminos Tradicionales y Paisaje Natural",
    "dek": "El Camino de la Aceña, el Vértice Andorra, Las Cavenes y la botánica autóctona del término.",
    "readingMinutes": 9,
    "headings": [
      {
        "slug": "el-camino-de-la-acena-y-la-concentracion-parcelaria",
        "text": "El Camino de la Aceña y la concentración parcelaria"
      },
      {
        "slug": "valdepega-y-la-toponimia-menor",
        "text": "Valdepega y la toponimia menor"
      },
      {
        "slug": "el-vertice-geodesico-andorra",
        "text": "El Vértice Geodésico \"Andorra\""
      },
      {
        "slug": "humedales-y-el-enigma-de-las-cavenes",
        "text": "Humedales y el enigma de Las Cavenes"
      },
      {
        "slug": "botanica-autoctona-zumacales-y-negrillos",
        "text": "Botánica autóctona: zumacales y negrillos"
      },
      {
        "slug": "el-esparto-y-miguel-el-silletero",
        "text": "El esparto y Miguel \"el silletero\""
      }
    ],
    "content": "## El Camino de la Aceña y la concentración parcelaria\n\nEntre las vías históricas del término destaca el **Camino de la Aceña** (o Camino de la Aceña de La Flecha), que conectaba el casco urbano con la vega del Tormes y el enclave molinero de La Flecha, en Cabrerizos. Nacía junto al antiguo frontón municipal —hoy carretera de Cabrerizos— y era corredor diario de mulas y carros cargados con costales de grano. A mediados del siglo XX, los planes de **concentración parcelaria** interrumpieron su trazado original en el límite con Cabrerizos, obligando a desviar el tránsito hacia el **Camino de Valdepega**.\n\n## Valdepega y la toponimia menor\n\nEl Camino de Valdepega atraviesa de norte a sur el sector meridional, hasta el pago de **Valdepega** («Valle de las Pegas», por la abundancia de urracas). Esta zona sostuvo una tradición vitivinícola hoy reducida a una única viña familiar superviviente. A lo largo del recorrido aparecen microtopónimos como **Carrelavieja** (antiguo camino en desuso), **La Pardaleja** (por los pardales o por el color pardo del suelo) y **La Andorra** (del árabe *al-Andurra*, terrenos boscosos o escabrosos).\n\n## El Vértice Geodésico \"Andorra\"\n\nSobre el paraje de La Andorra se alza el **Vértice Geodésico \"Andorra\"**, infraestructura del Instituto Geográfico Nacional catalogada con el número **47879**, construida el **1 de octubre de 1991**, con la célebre placa: *«La destrucción de esta señal está penada por la Ley»*. Constituye la máxima cota del término, a **871,464 metros**. Desde su cima se domina la penillanura, el valle del Tormes y la silueta de Salamanca, con un desnivel de **93 metros** hacia la vega del Tormes (778 m) y de **63 metros** respecto a la Plaza Mayor de Salamanca (808 m).\n\n## Humedales y el enigma de Las Cavenes\n\nEn la penillanura elevada se localiza la **Laguna de la Serrada** (o Charca de la Serrá), depresión endorreica estacional que fue objeto de una reforestación promovida por la Asociación de Cazadores local, creando un refugio para la perdiz roja. Hacia el sur, donde la penillanura se quiebra hacia el Tormes, aparece el paraje de **Las Cavenes** (o Las Cahenes): fosos, socavones y barrancos artificiales que una hipótesis defendida por investigadores locales como **Josito** vincula a antiguas explotaciones mineras romanas de oro a cielo abierto mediante el método del *ruina montium*, con paralelos en Las Médulas (León) y en Las Cavenes de El Cabaco (Sierra de Francia).\n\nDurante el siglo XX, el terreno sobrante entre el Camino de la Aceña y el de Valdepega acogió tiradas al plato durante las fiestas, y los socavones de Las Cavenes sirvieron de circuito de **motocross** en la década de 1980, antes de que la protección ambiental primase sobre estos usos.\n\n## Botánica autóctona: zumacales y negrillos\n\nLos **zumacales** (*Rhus Coriaria*) constituyen el elemento más singular del patrimonio botánico local: un arbusto de altísima concentración de taninos, empleado en el curtido de pieles y en la medicina popular como astringente y hemostático. Su cultivo, de marcada exclusividad territorial en la provincia, quedó representado en el **escudo municipal** junto al perfil de El Hoyo.\n\nLa arboleda tradicional estuvo representada casi en exclusiva por el **negrillo** u olmo común (*Ulmus minor*), con ejemplares monumentales como el de **Gallonegro** y el **árbol de Valdepega**. La llegada del ferrocarril en 1877 sumó hileras de negrillos junto a la Estación, y ya en el siglo XX la Política Agraria Común (PAC) impuso la reforestación de lindes, alterando la tradicional desnudez del secano armuñés.\n\n## El esparto y Miguel \"el silletero\"\n\nLos arroyos del término favorecieron el aprovechamiento etnobotánico del **esparto** (*Stipa tenacissima*), ligado a la memoria de **Miguel \"el silletero\"**, artesano que recolectaba y trenzaba la fibra para confeccionar los asientos de las sillas de las casas del pueblo y la comarca. Hoy, la pérdida de estos usos artesanales ha dejado los cauces cubiertos de maleza y juncales."
  },
  {
    "id": "04-economia-tradicional",
    "order": 4,
    "number": 4,
    "title": "La Economía Tradicional, Etnografía y Vida Cotidiana en La Armuña",
    "dek": "La molienda por maquila, la vitivinicultura de Valdepega y el agua blanda de La Flecha.",
    "readingMinutes": 11,
    "headings": [
      {
        "slug": "del-grano-a-la-acena-mulas-alforjas-y-las-eras",
        "text": "Del grano a la aceña: mulas, alforjas y las Eras"
      },
      {
        "slug": "el-sistema-de-la-maquila",
        "text": "El sistema de la maquila"
      },
      {
        "slug": "cereales-legumbres-y-la-tradicion-vinedera",
        "text": "Cereales, legumbres y la tradición viñedera"
      },
      {
        "slug": "yuntas-de-bueyes-y-pasto-invernal-en-el-tormes",
        "text": "Yuntas de bueyes y pasto invernal en el Tormes"
      },
      {
        "slug": "la-dualidad-del-agua-dureza-local-y-agua-blanda-de-la-flecha",
        "text": "La dualidad del agua: dureza local y agua blanda de La Flecha"
      },
      {
        "slug": "el-lavado-estival-y-las-ferias-de-salamanca",
        "text": "El lavado estival y las Ferias de Salamanca"
      }
    ],
    "content": "## Del grano a la aceña: mulas, alforjas y las Eras\n\nHasta hace entre 60 y 100 años, sin tractores, luz eléctrica ni vehículos a motor, el ciclo agrícola de Moriscos comenzaba en las **Eras**, a las afueras del pueblo. Allí, tras la trilla y el aventado estival, el grano se medía con la **media fanega** —recipiente de madera de forma colmenar— y se vertía en **costales** de lona o arpillera. Cargados a lomos de mulas con **alforjas** y aparejos de albarda, los labradores partían por el pontón de piedras de la Calzada de Medina y tomaban el Camino de la Aceña hacia el molino de **La Flecha**.\n\n## El sistema de la maquila\n\nEn la **Aceña de La Flecha**, molino hidráulico movido por el Tormes, el pago por la molienda no era en dinero, sino en especie: la **maquila** equivalía exactamente a una doceava parte (1/12) de la fanega, es decir, un **celemín**, extraído con el cazo llamado *maquilandero*. Esta retribución en harina convirtió a los molineros en grandes poseedores de materia prima sin ser propietarios agrícolas, y explica la estrecha relación gremial entre los molineros de La Flecha y los panaderos de Castellanos de Moriscos.\n\n## Cereales, legumbres y la tradición viñedera\n\nLa penillanura armuñesa fue uno de los graneros de la provincia: trigo candeal, cebada, centeno y, sobre todo, la **Lenteja de La Armuña** (hoy con Indicación Geográfica Protegida) y el **Garbanzo de Pedrosillo**. Menos conocida es la tradición vitivinícola de **Valdepega**, con parcelas de conducción baja para autoconsumo. La vendimia era ocasión de una célebre broma comunitaria: enviar a los niños a «pedir prestadas las escaleras de vendimiar al vecino», paralela a la novatada del «abrebocas» en la matanza del cerdo. Hoy solo sobrevive una viña familiar, cuyo vino artesanal se cata con humor bajo la denominación **«D.O. Valdepega»** en el histórico Bar de Chinarrilla (regentado por Narci).\n\n## Yuntas de bueyes y pasto invernal en el Tormes\n\nLos bueyes de raza negra o avileña roturaban las duras tierras arcillosas de La Armuña. En invierno, cuando el secano quedaba desnudo, los labradores conducían las yuntas hacia la vega del Tormes en La Flecha, donde el pasto era tierno y abundante. La memoria oral recuerda la casa de **Natalia y Damián**, últimos en mantener una pareja de bueyes hasta los años setenta, y la casa de **Paco \"El Rico\"** (hijo de Bernardo Escudero Daniel), que llegó a tener cinco parejas trabajando simultáneamente.\n\n## La dualidad del agua: dureza local y agua blanda de La Flecha\n\nLas aguas subterráneas de Moriscos, con más de 40º de dureza francesa, impedían cocer bien garbanzos y lentejas. Por ello, aprovechando los viajes a la aceña, las familias cargaban las mulas con cántaros para llenarlos de **agua blanda** en los manantiales de La Flecha, reservada para el cocido y las infusiones.\n\n## El lavado estival y las Ferias de Salamanca\n\nAl finalizar las labores de recolecta, a finales de agosto, mujeres y mozas organizaban la expedición anual del **lavado de ropa en el río**, descendiendo a los prados de La Flecha y Cabrerizos con coladas, artesas y jabón artesanal. Tras el aclarado, la ropa se extendía al sol en el «blanqueo», lista para las **Ferias y Fiestas de Salamanca** de septiembre, que marcaban el punto de inflexión antes de reiniciar el ciclo agrícola."
  },
  {
    "id": "05-despoblado-ribas-flecha",
    "order": 5,
    "number": 5,
    "title": "El Despoblado de Ribas y el Entorno de La Flecha: Historia, Mística y Patrimonio",
    "dek": "Del castillo de Ramiro II a la Granja Agustina, Fray Luis de León y el redescubrimiento de Unamuno.",
    "readingMinutes": 12,
    "headings": [
      {
        "slug": "la-fortaleza-de-ripas-de-ramiro-ii",
        "text": "La fortaleza de Ripas de Ramiro II"
      },
      {
        "slug": "la-granja-agustina-de-la-flecha-1451",
        "text": "La Granja Agustina de La Flecha (1451)"
      },
      {
        "slug": "fray-luis-de-leon-carcel-retiro-y-poesia",
        "text": "Fray Luis de León: cárcel, retiro y poesía"
      },
      {
        "slug": "de-la-desamortizacion-al-redescubrimiento-de-unamuno",
        "text": "De la Desamortización al redescubrimiento de Unamuno"
      },
      {
        "slug": "degradacion-y-lucha-por-la-conservacion",
        "text": "Degradación y lucha por la conservación"
      }
    ],
    "content": "## La fortaleza de Ripas de Ramiro II\n\nEn la vertiente meridional del término, donde la penillanura se quiebra hacia el Tormes, se localiza el **Despoblado de Ribas** (o Rivas). Tras la victoria cristiana sobre Abderramán III en la Batalla de Simancas (939), el rey **Ramiro II de León** fortificó la línea del Tormes, erigiendo entre otras plazas el **Castillo de Ribas**, sobre un cerro a 880 metros. Pese a las acometidas de Almanzor (977-986), Ribas mantuvo comunidad propia hasta que, con la estabilización de la frontera, su término se fragmentó en dos sectores: **Ribas Alto**, en torno al castillo —catalogado en 1752 como el «Mayorazgo del Caballero de Rivas», propiedad del catedrático José Galiano Enríquez de Navarra—, y **Ribas Bajo o La Flecha**, junto al río, ligada a las órdenes monásticas. Excavaciones informales del siglo XX, impulsadas por párrocos como Don Francisco de Dios, sacaron a la luz cimentaciones de piedra de Villamayor en la cima del **Terrubio**.\n\n## La Granja Agustina de La Flecha (1451)\n\nEn 1451, el religioso agustino **Fray Pedro Monroy** donó tierras al Convento de San Agustín de Salamanca, dando origen a la **Granja agustina de La Flecha**: un complejo agropecuario y centro de descanso estival para frailes y catedráticos. En el siglo XVI se dotó de una **Aceña** de cantería con el escudo de la Orden, un **Oratorio** en piedra de Villamayor (nave, capilla, sacristía y cripta) y huertas nutridas por el manantial de La Teja.\n\n## Fray Luis de León: cárcel, retiro y poesía\n\n**Fray Luis de León** (Belmonte, 1527 - Madrigal de las Altas Torres, 1591), catedrático de Teología en Salamanca, fue arrestado por la Inquisición en marzo de 1572 y encarcelado casi cinco años, acusado de traducir el Cantar de los Cantares sin licencia. Liberado en 1576, escribió en la celda las célebres décimas: *«Aquí la envidia y mentira / me tuvieron encerrado...»*. A su regreso a las aulas —con el mítico «Decíamos ayer...»— buscó refugio recurrente en la Granja de La Flecha, escenario de su **Oda a la vida retirada** («¡Qué descansada vida / la del que huye del mundanal rüido...!») y del arranque de su tratado *De los nombres de Cristo*, ambientado «por el mes de junio, a las vueltas de la fiesta de San Juan» en la ribera del Tormes.\n\n## De la Desamortización al redescubrimiento de Unamuno\n\nCon la Desamortización de Mendizábal (1835-1836), la finca pasó a manos privadas y, a finales del XIX, a los **Duques de Aveyro**, que restauraron el oratorio y lo convirtieron en panteón de los Carvajal. A comienzos del siglo XX, **Miguel de Unamuno**, rector de la Universidad de Salamanca, redescubrió el paraje como su «asiento de paz» y «refugio de verdura», glosándolo en *Paisajes* (1902) y en la *Oda a Salamanca* (1904): *«De la Flecha gozándose en la orilla... que el soto de Fray Luis cantando besa»*. Le acompañaron el fotógrafo José Suárez y su amigo **Casto Prieto Carrasco**, alcalde republicano de Salamanca (vínculo recreado en la película *Mientras dure la guerra* de Amenábar).\n\nEn 1953, la finca acogió el **Encuentro Nacional de Poetas**, consagrando La Flecha como el «Parnaso Ibérico». Por sus piedras pasaron los Nobel Jacinto Benavente, Camilo José Cela y Severo Ochoa, la poetisa Dulce María Loynaz, Carmen Martín Gaite y el cronista Gaziel.\n\n## Degradación y lucha por la conservación\n\nEl ferrocarril Salamanca-Madrid y la carretera SA-804 fracturaron la finca en **La Flecha Alta** (urbanizada con chalets) y **La Flecha Baja** (el oratorio y la aceña, abandonados). En los años setenta, una piscifactoría destruyó el cauce natural y el soto, y un chalet inacabado se levantó sobre la propia aceña. El expolio arrancó rejas, altares y losas. El **24 de octubre de 2011**, Hispania Nostra incluyó la Granja de La Flecha en su **Lista Roja del Patrimonio**, alertando del peligro de desplome del oratorio. Asociaciones vecinales de Cabrerizos y Moriscos, junto a iniciativas como *Literatura Abstracta*, reclaman su declaración como **Bien de Interés Cultural (BIC)**."
  },
  {
    "id": "06-conflictos-belicos",
    "order": 6,
    "number": 6,
    "title": "Conflictos Bélicos y Tensiones Históricas",
    "dek": "Bandos nobiliarios medievales, la Guerra de la Independencia de 1812 y la paradoja morisca de 1609.",
    "readingMinutes": 11,
    "headings": [
      {
        "slug": "bandos-nobiliarios-y-usurpaciones-medievales",
        "text": "Bandos nobiliarios y usurpaciones medievales"
      },
      {
        "slug": "la-guerra-de-la-independencia-1812",
        "text": "La Guerra de la Independencia (1812)"
      },
      {
        "slug": "la-minoria-morisca-y-la-paradoja-del-alfoz",
        "text": "La minoría morisca y la paradoja del alfoz"
      },
      {
        "slug": "el-suceso-de-agosto-de-1941",
        "text": "El suceso de agosto de 1941"
      }
    ],
    "content": "## Bandos nobiliarios y usurpaciones medievales\n\nDurante la Baja Edad Media, Salamanca se dividió en dos bandos nobiliarios —**San Benito** y **Santo Tomé**— que disputaban el control de la vega del Tormes: sus dehesas de pasto y sus aceñas. Los campesinos de Moriscos y Castellanos de Moriscos sufrieron durante generaciones las tropelías de estos bandos armados. El historiador **Nicolás Cabrillana** documentó cómo regidores como **Juan Vázquez Coronado** se apoderaron de las aceñas de Cabrerizos y La Flecha, cobrando la maquila con tarifas abusivas, mientras **Gonzalo de Villafuerte** monopolizaba los pastos de La Aldehuela y cerraba caminos con portazgos indebidos.\n\n## La Guerra de la Independencia (1812)\n\nEn el primer semestre de 1812, el **Ejército de Portugal** francés, al mando del mariscal **Auguste Marmont**, fortificó las elevaciones de Moriscos —**El Teso de La Cabaña** y **El Parapeto**— para proteger los accesos a Salamanca. En junio, el general **Arthur Wellesley** (futuro Duque de Wellington), con las guerrillas de **Julián Sánchez \"El Charro\"**, libró dos combates de tanteo en el término:\n\n- **20 de junio de 1812**: el Regimiento 68 de Infantería Ligera británico resistió el asalto francés en el casco urbano hasta el anochecer, con unas 50 bajas.\n- **22 de junio de 1812**: Wellington desalojó a la baioneta las posiciones de El Parapeto y La Cabaña, con otras 50 bajas, forzando la retirada de Marmont hacia Aldearrubia y Huerta.\n\nUn mes después, el **22 de julio de 1812**, se libró la decisiva **Batalla de Los Arapiles**. El párroco **Francisco Palomero Ábila** dejó constancia manuscrita del saqueo sufrido por el pueblo: *«...asaltaron las tropas del Emperador las casas de este infeliz pueblo, saqueando e incendiando cuanto hallaron a su paso...»*. Las familias huyeron a los sotos de La Flecha y a Las Cavenes. La tradición local vincula este episodio con la leyenda del **Cristo de las Batallas**, salvado por un pastor que lo escondió en la vega ante el avance francés.\n\n## La minoría morisca y la paradoja del alfoz\n\nTras la Rebelión de las Alpujarras (1568-1570), sofocada por Don Juan de Austria, la Corona deportó a más de 60.000 moriscos granadinos hacia Castilla. El **Decreto de Expulsión General** de Felipe III y el Duque de Lerma, firmado el **9 de abril de 1609**, afectó a más de 350.000 personas. Aquí radica la gran paradoja historiográfica: cuando el Conde de Salazar aplicó la expulsión en Salamanca a partir de 1610, la villa de **Moriscos no sufrió despoblación alguna**, pues su nombre medieval («Morisco», documentado desde el siglo XIII-XIV) nada tenía que ver con los conversos granadinos recién deportados. El contraste es notable con Ciudad Rodrigo, donde entre 1610 y 1616 se persiguió a más de cincuenta familias de moriscos procedentes de Baza, Huéscar y Purchena.\n\n## El suceso de agosto de 1941\n\nLa noche del **15 de agosto de 1941**, festividad de la Asunción, las hermanas **Luisa** (20 años) y **Josefa Salvador Domínguez** (17 años), junto a su criada **Benita**, regresaban a pie de las verbenas de Aldealengua cuando una figura embozada en capa oscura y careta blanca las atacó con una **horca de hierro de cuatro pinchos** robada esa misma tarde. Benita logró huir y alertar al pueblo; las hermanas, con entre 25 y 26 heridas de punzón, fueron trasladadas de urgencia a Salamanca y sobrevivieron. La Guardia Civil identificó al autor por el apero abandonado —con un diente doblado— como **Marino Poveda Pedraz**, antiguo criado de la familia, que confesó actuar por despecho hacia Luisa. La Audiencia Provincial le condenó a más de 16 años de prisión por alevosía, nocturnidad, uso de disfraz y desprecio del sexo (precedente de la actual violencia de género). El apero, conocido desde entonces como **«la horca de Marino»**, se conservó durante décadas en la casa donde el agresor servía. Este suceso se aborda con más detalle, junto a su eco en la memoria civil contemporánea, en el capítulo siguiente."
  },
  {
    "id": "07-sucesos-cronica-negra",
    "order": 7,
    "number": 7,
    "title": "Sucesos, Crónica Negra y Memoria Civil Local",
    "dek": "La emboscada de 1941, la 'horca de Marino' y la tragedia del Alvia de 2025.",
    "readingMinutes": 10,
    "headings": [
      {
        "slug": "la-emboscada-de-la-noche-de-agosto-de-1941",
        "text": "La emboscada de la noche de agosto de 1941"
      },
      {
        "slug": "el-ataque-y-la-huida-de-benita",
        "text": "El ataque y la huida de Benita"
      },
      {
        "slug": "investigacion-condena-y-mito-oral",
        "text": "Investigación, condena y mito oral"
      },
      {
        "slug": "la-trinchera-del-ferrocarril-un-siglo-y-medio-de-riesgo",
        "text": "La trinchera del ferrocarril: un siglo y medio de riesgo"
      },
      {
        "slug": "la-tragedia-del-alvia-julio-de-2025",
        "text": "La tragedia del Alvia (julio de 2025)"
      }
    ],
    "content": "## La emboscada de la noche de agosto de 1941\n\nEn la calurosa noche del **15 de agosto de 1941**, mientras Aldealengua celebraba la Asunción de la Virgen con verbenas y bailes, tres jóvenes de Moriscos —las hermanas **Luisa** (20 años) y **Josefa Salvador Domínguez** (17 años) y su criada **Benita**— emprendieron a pie el camino de regreso. Hacia las diez de la noche, en mitad de la oscuridad del camino rural, una figura embozada en gruesa **capa oscura** y **careta blanca** de cartón emergió de la maleza y se abalanzó sin mediar palabra sobre las jóvenes, empuñando un pesado **horcón de hierro de cuatro pinchos** robado esa misma tarde de una finca cercana.\n\n## El ataque y la huida de Benita\n\nEl agresor golpeó repetidamente a Luisa y Josefa, infligiéndoles entre **25 y 26 heridas de punzón** en brazos, tórax y rostro; uno de los dientes del horcón quedó doblado por la violencia de los golpes, dando origen al nombre con el que se recuerda el arma: **«la horca de Marino»**. Aprovechando la confusión, Benita logró escapar y corrió por los campos hasta el pueblo, alertando al padre de las víctimas, **Francisco Salvador**. Una comitiva de vecinos con faroles trasladó a las hermanas, malheridas, en un carro mullido con paja hasta el hospital provincial de Salamanca, donde lograron sobrevivir tras varias semanas de cuidados.\n\n## Investigación, condena y mito oral\n\nLa Guardia Civil halló el horcón abandonado junto al camino y comprobó que había sido sustraído esa tarde de una finca próxima, junto con una caballería. Las pesquisas condujeron a **Marino Poveda Pedraz**, antiguo mozo de cuadra en casa de Francisco Salvador, quien confesó actuar por despecho hacia Luisa, a quien acusaba de haberle humillado durante su servicio. El diario *El Adelanto* recogió el caso el **17 de agosto de 1941**. La Audiencia Provincial de Salamanca le condenó a **8 años y un día de prisión mayor por cada uno de los dos delitos** (más de 16 años en total) y una indemnización de 500 pesetas, apreciando alevosía y nocturnidad, uso de disfraz y desprecio del sexo —agravante precursora de la actual tipificación de la violencia de género—. Tras cumplir condena, Marino regresó y trabajó como peón en el ferrocarril, de donde nació el dicho socarrón con el que los ancianos cerraban el relato: tras salir de la cárcel, «se puso al tren».\n\n## La trinchera del ferrocarril: un siglo y medio de riesgo\n\nDesde la inauguración de la línea **Salamanca-Medina del Campo** el 26 de agosto de 1877 y la puesta en servicio del apeadero de Moriscos (punto kilométrico 69,380), la vía ha sido un motor de desarrollo pero también una fuente constante de riesgo: pasos a nivel sin barrera, atropellos de ganado y accidentes que llevaron a solicitar reiteradamente a RENFE y al Ministerio de Fomento la supresión de estos puntos negros.\n\n## La tragedia del Alvia (julio de 2025)\n\nEn la madrugada del **21 de julio de 2025**, un tren Alvia Madrid-Salamanca arrolló mortalmente a dos personas —un hombre de 41 años y una mujer de 43, pareja sentimental— en el tramo entre Gomecello y Moriscos. El maquinista los divisó forcejeando sobre la vía segundos antes del impacto. El convoy, con 257 pasajeros a bordo, quedó detenido más de tres horas junto al apeadero. La investigación reveló que la mujer había denunciado a su pareja en mayo de 2025 por malos tratos, con expediente activo de riesgo bajo en el **Sistema VioGén**. El suceso, asumido por el Juzgado de Instrucción nº1 de Salamanca, unió de forma dolorosa la memoria civil del municipio con la lacra de la violencia de género y el histórico peligro de la trinchera ferroviaria."
  },
  {
    "id": "08-evolucion-infraestructural",
    "order": 8,
    "number": 8,
    "title": "Evolución Infraestructural, Demográfica y Patrimonial (Siglos XIX-XXI)",
    "dek": "El ferrocarril de 1877, el tren de rescate de 1890 y el renacer de Moriscos como cinturón metropolitano.",
    "readingMinutes": 10,
    "headings": [
      {
        "slug": "la-revolucion-del-vapor-la-linea-de-1877",
        "text": "La revolución del vapor: la línea de 1877"
      },
      {
        "slug": "el-tren-de-rescate-de-1890",
        "text": "El tren de rescate de 1890"
      },
      {
        "slug": "el-declive-de-la-sociedad-agropastoril",
        "text": "El declive de la sociedad agropastoril"
      },
      {
        "slug": "del-secano-cerealista-al-cinturon-residencial",
        "text": "Del secano cerealista al cinturón residencial"
      },
      {
        "slug": "el-renacer-del-patrimonio-rutas-armunesas-y-ruta-nocturna",
        "text": "El renacer del patrimonio: Rutas Armuñesas y Ruta Nocturna"
      }
    ],
    "content": "## La revolución del vapor: la línea de 1877\n\nEl **26 de agosto de 1877** se inauguró el tramo ferroviario entre El Pedroso de la Armuña y Salamanca, dentro de la línea **Medina del Campo-Salamanca**. La **Estación de Moriscos**, en el punto kilométrico **69,380**, dio salida masiva al trigo, la cebada y la lenteja armuñesa hacia los mercados nacionales, insertando a Moriscos en los circuitos comerciales. En torno al apeadero surgió el núcleo secundario de **La Estación**, con almacenes, tinglados y viviendas de ferroviarios, integrado en 1928 en la Compañía Nacional de los Ferrocarriles del Oeste y nacionalizado en 1941 bajo RENFE (hoy Adif/Renfe Viajeros). La mecanización y automatización acabaron convirtiéndolo en un despoblado.\n\n## El tren de rescate de 1890\n\nEl **1 de octubre de 1890**, una tormenta de pedrisco de proporciones apocalípticas destruyó cosechas, tapias y caminos, aislando por completo al pueblo. El Gobernador Civil de Salamanca organizó un **tren especial de rescate** cargado de mantas, medicinas y víveres que se detuvo en la Estación de Moriscos para socorrer a la población damnificada, un episodio que quedó grabado como la cara más humana del ferrocarril.\n\n## El declive de la sociedad agropastoril\n\nA lo largo del siglo XX se extinguió el ritmo ancestral del campo: las yuntas de bueyes cedieron ante los tractores, los viajes a la Aceña de La Flecha para la maquila terminaron con el agua corriente en las casas, y la mecanización redujo la mano de obra necesaria, desencadenando entre los años sesenta y ochenta un éxodo rural hacia Salamanca, Madrid y Cataluña.\n\n## Del secano cerealista al cinturón residencial\n\nA las puertas del siglo XXI, la proximidad a Salamanca —9 km— revirtió la tendencia. Las cifras del INE lo certifican:\n\n- Mediados del siglo XIX (Diccionario de Madoz): 37 vecinos pecheros, 173 almas.\n- Año 2018: 384 habitantes.\n- Año 2021: 487 habitantes (densidad de 37,99 hab/km²).\n- Año 2025 (estimación INE): **577 habitantes**, en crecimiento continuo.\n\nEl despliegue de **fibra óptica ultrarrápida** (programa UNICO Banda Ancha) ha permitido el teletrabajo desde el pueblo, y la Mancomunidad La Armuña ha dotado al municipio de pabellones polideportivos, parques infantiles y zonas ajardinadas sobre la antigua charca comunal.\n\n## El renacer del patrimonio: Rutas Armuñesas y Ruta Nocturna\n\nMoriscos impulsa hoy una red de siete **Rutas Armuñesas** de BTT, senderismo y gravel, con salida en el recinto de la I.G.P. Lenteja de La Armuña, que recorren la Vega y los Escarpes, el Vértice Andorra y Las Cavenes. Pero la iniciativa cultural de mayor calado identitario es la **Ruta Nocturna a La Flecha**, promovida por la **Asociación Cultural de Mujeres de Moriscos** con el Ayuntamiento: una marcha bajo la luna llena estival de 7,7 km que, con paradas locutadas por cronistas como **Josito**, recorre las eras, el vértice Andorra, los escenarios de 1812 y la emboscada de 1941, culminando con el recitado de la *Oda a la vida retirada* junto al oratorio de La Flecha."
  },
  {
    "id": "09-glosario-anexos",
    "order": 9,
    "number": 9,
    "title": "Glosario Etnográfico, Anexos y Memoria Documental",
    "dek": "El habla de la tierra, la guía de campo de la Ruta Nocturna y las fuentes que sostienen esta obra.",
    "readingMinutes": 8,
    "headings": [
      {
        "slug": "el-habla-de-la-tierra",
        "text": "El habla de la tierra"
      },
      {
        "slug": "guia-de-campo-de-la-ruta-nocturna-a-la-flecha",
        "text": "Guía de campo de la Ruta Nocturna a La Flecha"
      },
      {
        "slug": "fuentes-documentales-y-agradecimientos",
        "text": "Fuentes documentales y agradecimientos"
      }
    ],
    "content": "## El habla de la tierra\n\nEl léxico tradicional de Moriscos es un tesauro mestizo, heredero de la repoblación leonesa, el sustrato mudéjar y la vida agrícola del secano armuñés. Palabras como **alforjas**, **costal**, **fanega y media fanega**, **celemín**, **maquila**, **horcón**, **pontón**, **vertedera**, **negrillo**, **zumacales** o **cavenes** —desarrolladas con detalle en la sección de Glosario de este sitio— no son curiosidades filológicas, sino la memoria viva de una economía que midió la tierra y el grano con precisión artesanal durante siglos. A ellas se suma la **Lenteja de La Armuña**, cultivo insignia hoy protegido por la Indicación Geográfica Protegida, y el concepto lírico del **«asiento de paz»**, el rincón junto al Tormes donde Unamuno se sentaba a meditar en comunión con Fray Luis de León.\n\n## Guía de campo de la Ruta Nocturna a La Flecha\n\nEl itinerario de **7,7 kilómetros**, de dificultad baja y perfil descendente, articula ocho hitos que resumen la historia de Moriscos:\n\n1. **Salida del casco urbano y el antiguo Frontón**, junto al recuerdo de la charca municipal.\n2. **Las Eras y el pago de Valdepega**, evocando la trilla con la media fanega y la tradición vinícola.\n3. **La Charca de la Serrá y los arroyos**, con la confluencia de Los Moriscos, el Vallejón, Gandil y Serranilla.\n4. **El Vértice Geodésico \"Andorra\"** (871,4 m), con vistas nocturnas a las torres iluminadas de la Catedral de Salamanca.\n5. **Los escenarios bélicos de 1812**: El Teso de La Cabaña y El Parapeto.\n6. **Las Cavenes y la fortaleza de Ripas**, con las huellas del lavado de arenas auríferas romanas.\n7. **La Pesquera y la Aceña agustina**, donde se cobraba la maquila y se lavaba la ropa en verano.\n8. **El Oratorio del siglo XVI y el Sillón de Unamuno**, donde se recita bajo las estrellas la *Oda a la vida retirada*.\n\n## Fuentes documentales y agradecimientos\n\nEsta obra se sostiene sobre el cruce de fuentes primarias y secundarias: los registros del **Archivo Histórico de la Catedral de Salamanca** (con el *Libro de todos los préstamos* de 1265), el **Catastro del Marqués de la Ensenada** (1752, Archivo General de Simancas), el *Diccionario geográfico-estadístico-histórico* de **Pascual Madoz** (1845-1850), las obras de **Fray Luis de León** y **Miguel de Unamuno**, los estudios de hispanistas como Antonio Domínguez Ortiz, Bernard Vincent, Ángel Barrios, Nicolás Cabrillana o Pérez Bustamante, y la hemeroteca de *El Adelanto* y RTVE.\n\nUn reconocimiento entrañable a la **memoria oral de las familias de Moriscos**, a los abuelos y abuelas que conservaron el nombre de las yuntas, el secreto del zumaque y los relatos de 1941; y de manera muy especial a la **Asociación Cultural de Mujeres de Moriscos**, promotora de la Ruta Nocturna, al **Ayuntamiento de Moriscos** y a divulgadores locales como **Josito**, cuya voz e investigación mantienen viva la memoria de este pueblo de La Armuña."
  },
  {
    "id": "10-epilogo-fuentes",
    "order": 10,
    "number": 10,
    "title": "Epílogo, Memoria Viva y Fuentes Documentales",
    "dek": "El hilo invisible que une diez siglos de historia en la penillanura de La Armuña.",
    "readingMinutes": 6,
    "headings": [
      {
        "slug": "la-convergencia-de-las-eras-en-la-penillanura",
        "text": "La convergencia de las eras en la penillanura"
      },
      {
        "slug": "la-guardia-del-recuerdo",
        "text": "La guardia del recuerdo"
      },
      {
        "slug": "bibliografia-comentada-las-joyas-del-archivo",
        "text": "Bibliografía comentada: las joyas del archivo"
      }
    ],
    "content": "## La convergencia de las eras en la penillanura\n\nHay algo sagrado en la forma en que el tiempo se deposita sobre la tierra llana de La Armuña. Sin grandes cordilleras que oculten el horizonte, la historia no se entierra del todo: permanece en el viento que peina las espigas y en el polvo de los caminos hacia el Tormes. Por las mismas veredas por las que hoy pasean los niños del pueblo transitaron, en 1100, los repobladores del conde Vela de Aragón y los antiguos campesinos de estirpe mudéjar que legaron su nombre al lugar. En El Parapeto y La Cabaña, donde retumbó la artillería de Wellington y Marmont en 1812, brotan hoy flores silvestres. Y en los escarpes de La Flecha resuena todavía el eco de los pasos de Fray Luis de León buscando la paz del «huerto plantado», mientras sobre las piedras de la Aceña perdura el sudor de generaciones de molineros que midieron la vida a golpes de maquila y celemín. Incluso el dolor de la noche de agosto de 1941 forma parte de ese mismo hilo invisible: la constancia de que este pueblo ha sabido resistir, sanar sus heridas y seguir caminando erguido.\n\n## La guardia del recuerdo\n\nLa memoria histórica no es un monumento inmóvil, sino un acto de fe diario que solo permanece encendido si hay manos dispuestas a pasarlo de generación en generación. Cada verano, cuando los vecinos se calzan las botas para recorrer la **Ruta Nocturna a La Flecha** —iniciativa de la Asociación Cultural de Mujeres de Moriscos y el Ayuntamiento—, no realizan un simple paseo nocturno: celebran un ritual de permanencia. Al pisar el Camino de la Aceña, detenerse en la Charca de la Serrá, divisar Salamanca desde el Vértice Andorra y recitar los versos de Unamuno junto al Tormes, los caminantes de hoy se convierten en herederos de la historia. Dicen los ancianos que los muertos no se van del todo mientras haya alguien que pronuncie sus nombres.\n\n## Bibliografía comentada: las joyas del archivo\n\nCuatro fuentes documentales vertebran especialmente esta obra. El **Catastro del Marqués de la Ensenada** (1752) ofrece una fotografía de alta resolución del Moriscos del siglo XVIII: casas habitadas, términos redondos, huebras de secano y cabezas de ganado. El **«Libro de todos los préstamos»** de la Catedral de Salamanca (1265) es el acta de nacimiento paleográfica del topónimo, redactado bajo Alfonso X el Sabio. La hemeroteca de ***El Adelanto*** (agosto de 1941) transforma la crónica negra en una lección de historia social. Y la cartografía del **Instituto Geográfico Nacional** —incluida la hoja 478 de Salamanca— permite medir con exactitud milimétrica la altitud del Vértice Andorra (871,464 m) y reconstruir los desniveles hacia el Tormes.\n\nCon la luz de estos documentos y el latido incesante de la memoria oral se cierra este libro. Quede en manos de los lectores de hoy y de los vecinos de mañana la tarea de cuidar este legado, seguir pisando los caminos de Moriscos y recordar que bajo el suelo de La Armuña late la historia viva de nuestra tierra."
  },
  {
    "id": "11-portal-morisquenos",
    "order": 11,
    "number": 11,
    "title": "La Memoria Digital de la Villa: Análisis del Portal 'Morisqueños'",
    "dek": "El Bosque Genealógico desde 1645, la desmitificación del topónimo y la galería de paisanos ilustres.",
    "readingMinutes": 10,
    "headings": [
      {
        "slug": "el-proyecto-digital-morisquenos-y-su-marco-institucional",
        "text": "El proyecto digital \"Morisqueños\" y su marco institucional"
      },
      {
        "slug": "la-distincion-de-morisqueno-y-el-bosque-genealogico-1645",
        "text": "La Distinción de Morisqueño y el Bosque Genealógico (1645)"
      },
      {
        "slug": "la-desmitificacion-del-toponimo",
        "text": "La desmitificación del topónimo"
      },
      {
        "slug": "prosopografia-local-la-galeria-de-paisanos-ilustres",
        "text": "Prosopografía local: la galería de Paisanos Ilustres"
      },
      {
        "slug": "patrimonio-devocional-y-el-baul-etnografico",
        "text": "Patrimonio devocional y el Baúl etnográfico"
      },
      {
        "slug": "hemeroteca-y-evolucion-urbana",
        "text": "Hemeroteca y evolución urbana"
      }
    ],
    "content": "## El proyecto digital \"Morisqueños\" y su marco institucional\n\nLa conservación y divulgación del patrimonio de Moriscos ha encontrado su principal precedente en la plataforma **«Morisqueños - Memoria e historia de Moriscos»** (moriscos.info), un proyecto pionero de microhistoria participativa articulado en torno a tres objetivos: servir de punto de encuentro para vecinos, emigrados y simpatizantes; investigar y divulgar la historia y etnografía del municipio; y canalizar la colaboración ciudadana mediante buzón de correo, libro de visitas y foros.\n\n## La Distinción de Morisqueño y el Bosque Genealógico (1645)\n\nSe define como **«morisqueño»** a toda persona con un lazo de unión —residencial, genealógico, histórico o afectivo— con el pueblo. Esta condición se acredita incorporándose al **Bosque Genealógico de Moriscos** (geneaweb.org/moriscos), elaborado mediante el vaciado sistemático de los libros sacramentales de la Iglesia de San Pedro Apóstol, que permite rastrear los linajes familiares ininterrumpidamente hasta el año **1645**. La adscripción da además derecho a figurar en el archivo fotográfico **«Ventanas del Ayer y Hoy»**, con más de 250 morisqueños retratados desde 1930.\n\n## La desmitificación del topónimo\n\nUno de los aportes historiográficos más relevantes del portal es la refutación de la creencia popular que atribuye el nombre del pueblo a la Rebelión de las Alpujarras o al decreto de expulsión de 1609. El cotejo paleográfico de los pergaminos del Archivo Catedralicio de Salamanca y del Archivo General de Simancas confirma que la aldea ya existía bajo la denominación singular «Morisco» durante la Baja Edad Media. La cronología documental que sostiene esta refutación incluye hitos como:\n\n- **1102**: inicio de la colonización del alfoz por Ramón de Borgoña y el obispo Jerónimo de Perigord.\n- **4 de octubre de 1164**: primera mención explícita, *«collationam Sancti Christofori de Morisco»*, con 56 vecinos pecheros y parroquia de San Pedro.\n- **1265**: registro en el *Libro de todos los préstamos* como «Morisco de Valdevilloria».\n- **1289**: venta de parcelas entre vecinos de Castellanos de Morisco, confirmando la anterioridad de Moriscos respecto a la puebla de castellanos.\n- **Censos del siglo XVI**: 106 vecinos en 1591 (103 pecheros, 1 hidalgo, 2 clérigos).\n- **1752**: Catastro del Marqués de la Ensenada.\n- **1877**: inauguración del ferrocarril y la Estación de Moriscos.\n\n## Prosopografía local: la galería de Paisanos Ilustres\n\nEl portal rescata a tres figuras que este sitio recoge también en su sección de **Genealogía**: **Sebastián Romo**, clérigo del siglo XVII defensor de los derechos del clero rural armuñés; **Eugenio Blanco Carbayo** (1858-1935), maestro del arte naïf armuñés, escultor e imaginero autodidacta; y **Álex Herrera Pedraz** (1949), investigador y responsable del archivo fotográfico y de la sección **«DONes y ELes»**, un estudio de los tratamientos de cortesía en los padrones de los siglos XVII a XIX.\n\n## Patrimonio devocional y el Baúl etnográfico\n\nEl portal documenta la **Iglesia de San Pedro Apóstol**, con sillares de piedra de Villamayor y restos de tradición mudéjar, cuyas tablas del siglo XVI se custodian hoy en el Museo de Bellas Artes de Salamanca. El lienzo de la **Virgen Peregrina**, patrona de Moriscos, fue restaurado el 6 de agosto de 2016 por los talleres Uffizzi, gracias al mecenazgo de Antonio García Malmierca. Las fiestas patronales se celebran entre el 30 de julio y el 2 de agosto. Bajo el epígrafe **«El Baúl»**, el portal reúne coplas de la trilla, refranero agrícola y romancero de bodas, junto a la catalogación del **Museo Etnográfico de Hipólito**, con yugos, trillos, medias fanegas, celemines y el maquilandero de la Aceña de La Flecha.\n\n## Hemeroteca y evolución urbana\n\nCompleta el archivo una hemeroteca con recortes de *El Adelanto*, *Tribuna de Salamanca* y *Salamanca24horas* —desde la supresión de pasos a nivel en 2003 hasta el IV MotoCross de Moriscos en Las Cavenes (2009)— y una serie de ortofotos de **evolución urbana entre 1954 y 2007**, testigo de la transición del caserío tradicional a las modernas urbanizaciones. Esta plataforma —de la que el presente sitio es heredero y continuador— se consagra así como modelo de salvaguarda de la identidad viva de Moriscos para las generaciones futuras."
  }
];
